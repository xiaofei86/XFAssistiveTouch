...
- (void)loadView {
    ...
    _effectView = [[UIView alloc] initWithFrame:_contentView.bounds];
    ...
    _contentItem = [XFATItemView itemWithImage:[UIImage imageNamed:@"m1"]];
...
- (void)spread {
    ...
    CGRect contentFrame = [XFATLayoutAttributes contentViewSpreadFrame:self.contentView.frame];
    BOOL toRight = CGRectGetMinX(contentFrame) == CGRectGetMinX(self.contentItem.frame);
    for (int i = 0; i < count; i++) {
        ...
        [UIView animateWithDuration:[XFATLayoutAttributes animationDuration] animations:^{
            if (toRight) {
                item.center = CGPointMake(CGRectGetMidX(self.contentItem.frame) + [XFATLayoutAttributes itemWidth] * (i + 1), CGRectGetMidY(contentFrame));
            } else {
                item.center = CGPointMake(CGRectGetMidX(self.contentItem.frame) - [XFATLayoutAttributes itemWidth] * (i + 1), CGRectGetMidY(contentFrame));
            }
            item.alpha = 1;
...
- (void)pushViewController:(XFATViewController *)viewController atPisition:(XFATPosition *)position {
    ...
    for (int i = 0; i < count; i++) {
        ...
        [self.view addSubview:item];
        [UIView animateWithDuration:[XFATLayoutAttributes animationDuration] animations:^{
            item.center = [XFATPosition positionWithCount:count index:i referenceFrame:self.contentItem.frame].center;
            item.alpha = 1;
        }];
    }
